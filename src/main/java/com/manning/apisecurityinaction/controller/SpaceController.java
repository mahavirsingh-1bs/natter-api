...
public class SpaceController {
	...
	public Message readMessage(Request request, Response response) {
		...
		var message = database.findUnique(Message.class, "SELECT space_id, msg_id, author, msg_time, msg_text "
	...
	public JSONArray findMessages(Request request, Response response) {
		...
		var messages = database.findAll(Long.class,
				"SELECT msg_id FROM messages WHERE space_id = ? AND msg_time >= ?;", spaceId, since);

		response.status(200);
		return new JSONArray(messages.stream().map(msgId -> "/spaces/" + spaceId + "/messages/" + msgId)
	...
	public JSONObject addMember(Request request, Response response) {
		...
		database.updateUnique("INSERT INTO permissions(space_id, user_id, perms) " + "VALUES(?, ?, ?)", spaceId,
				userToAdd, perms);
